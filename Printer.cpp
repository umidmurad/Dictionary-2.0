...
{       //this function needs another function to check if the pos actually is not random word.
    ...
    if(!check) { cout << "|"; return; }// if the word and pos was founded and printed, do not try to check for errors (line 114-116), just return back to main
...
void distincthelper(vector<string> optionHolder){
    ...
    if(tempvec.size()>1) {
        for (int i = 1; i < tempvec.size(); i++) {
            if ((tempvec[j].first ==tempvec[i].first) && (tempvec[j].second.first==tempvec[i].second.first)&&(tempvec[j].second.second==tempvec[i].second.second)){
